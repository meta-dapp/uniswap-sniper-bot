    ...
    Web3Wss.subscribePairCreated(Contract, config, async (pairs) => {
        for (var i = 0; i < pairs.length; i++) {
            ...
            if (pair.liquidity.good_liquidity) {
                if (pair.token0.toLowerCase() === config.wethContract.toLowerCase()
                    || pair.token1.toLowerCase() === config.wethContract.toLowerCase()) {
                    ...
                    pair['liquidity'] = pair.liquidity.eth
...
const Sell = async (token, _store) => {
    ...
    const ContractPCS = (await Contract.Instance(Types.ROUTER, config.usRouterContract, config)).methods
    const ContractToken = (await Contract.Instance(Types.TOKEN, token.address, config)).methods
    const tokenIn = token.address
    const tokenOut = config.wethContract
    ...
    const allowance = await ContractToken.allowance(config.userAddress, config.usRouterContract).call()
    ...
    if (tx.status) {
        ...
        var amountOutMin = Decode.FromWei(amounts[amounts.length - 1], config.wethContract)
        ...
        log(chalk.red(`
         ...
         amountOut: ${parseFloat(expectedAmount)} ${tokenOut} WETH
       `))
...
const Buy = async (token, _store) => {
    ...
    const ContractPCS = (await Contract.Instance(Types.ROUTER, config.usRouterContract, config)).methods
    const ContractWETH = (await Contract.Instance(Types.TOKEN, config.wethContract, config)).methods
    const tokenIn = config.wethContract
    const tokenOut = token.address

    const amountIn = Decode.ToWei(amount, 18)

    log(chalk.yellow('Checking WETH allowance...'))
    var tx = {}
    const supply = await ContractWETH.totalSupply().call()
    const allowance = await ContractWETH.allowance(config.userAddress, config.usRouterContract).call()
    ...
    if (tx.status) {
        ...
        log(chalk.green(`
         Buying ${tokenOut}
         =================
         amountIn: ${amount.toString()} ${tokenIn} WETH
