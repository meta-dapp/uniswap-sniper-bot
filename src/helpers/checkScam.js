...
const getPairAddress = async (config, tokenAddress) => {
    const factory = (await Contract.Instance(types.FACTORY, config.usFactoryContract, config)).methods
    return await factory.getPair(tokenAddress, config.wethContract).call()
}
